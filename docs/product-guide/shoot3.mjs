...
// The "Start recall" form only — opened, never submitted. Nothing is written.
await go("/recalls");
await sleep(3500);
const opened = await evaluate(`(() => {
  const btn = [...document.querySelectorAll('button')].find(b => /start recall/i.test(b.innerText));
  if (!btn) return "no button";
  btn.click();
  return "opened";
})()`);
console.log("recall form:", opened);
await sleep(1800);
await shoot("recall-form");
